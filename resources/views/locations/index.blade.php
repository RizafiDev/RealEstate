...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="text-gray-600">Manage project locations</p>
        </div>
        <a href="{{ route('locations.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="bg-white overflow-hidden shadow rounded-lg">
            <div class="p-5">
                <div class="flex items-center">
                    <div class="flex-shrink-0">
                        <i class="fas fa-city text-gray-400 text-xl"></i>
                    </div>
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                            <dt class="text-sm font-medium text-gray-500 truncate">Cities</dt>
                            <dd class="text-lg font-medium text-gray-900">{{ $stats['cities'] }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white overflow-hidden shadow rounded-lg">
            <div class="p-5">
                <div class="flex items-center">
                    ...
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                            <dt class="text-sm font-medium text-gray-500 truncate">Provinces</dt>
                            <dd class="text-lg font-medium text-gray-900">{{ $stats['provinces'] }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
        ...
        <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <select name="type" id="type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                    ...
                    <option value="district" {{ request('type') == 'district' ? 'selected' : '' }}>District</option>
                    <option value="subdistrict" {{ request('type') == 'subdistrict' ? 'selected' : '' }}>Subdistrict</option>
                </select>
            </div>
                    ...
                    <li class="px-6 py-4 hover:bg-gray-50">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                @if($location->address)
                                    <p class="text-sm text-gray-600 mt-1">{{ Str::limit($location->address, 100) }}</p>
                                @endif
                            </div>
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('locations.show', $location) }}" class="text-indigo-600 hover:text-indigo-900">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <a href="{{ route('locations.edit', $location) }}" class="text-yellow-600 hover:text-yellow-900">
